  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl md:text-3xl font-bold text-gray-900">{getPageTitle()}</h1>
          <p className="text-sm md:text-base text-gray-600">{getPageDescription()}</p>
        </div>
        <div className="flex flex-wrap items-center gap-2 md:gap-4">
          {/* Global Status */}
          <div className="flex items-center gap-1 md:gap-2">
            <span className="text-xs md:text-sm font-medium text-gray-700">Global Status:</span>
            <div className="flex items-center gap-1 md:gap-2">
              <div className={`w-2 h-2 md:w-3 md:h-3 rounded-full ${isGloballyActive ? 'bg-green-500' : 'bg-gray-400'}`}></div>
              <span className={`text-xs md:text-sm font-medium ${isGloballyActive ? 'text-green-600' : 'text-gray-500'}`}>
          ...
          {isGloballyActive ? (
            <Button 
              ...
              className="bg-primary text-white hover:bg-blue-700 h-8 md:h-10 text-xs md:text-sm px-2 md:px-4"
            >
            ...
            <Button 
              ...
              className="bg-green-600 text-white hover:bg-green-700 h-8 md:h-10 text-xs md:text-sm px-2 md:px-4"
            >
          ...
          <Button 
            variant="outline" 
            className="text-gray-700 h-8 md:h-10 text-xs md:text-sm px-2 md:px-4"
            onClick={() => toast({
              title: "Admin Panel",
              description: "Admin features accessible via Telegram bot",
            })}
          >
            <UserCircle className="w-3 h-3 md:w-4 md:h-4 mr-1 md:mr-2" />
            Admin
          </Button>
        </div>
      </div>
